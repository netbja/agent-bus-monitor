...
func seedSpaced(t *testing.T, b *Bus, entries [][2]string) {
	...
	for _, e := range entries {
		...
		switch e[0] {
		case "status":
			_, err = b.Status(ctx, "dev", "working", e[1], AgentIdent{})
		case "report":
...
func TestTailFromDoesNotReplay(t *testing.T) {
	...
	if _, err := b.Status(ctx, "dev", "working", "old", AgentIdent{}); err != nil {
		t.Fatalf("seed: %v", err)
	}
	...
	if _, err := b.Status(ctx, "dev", "idle", "new", AgentIdent{}); err != nil {
